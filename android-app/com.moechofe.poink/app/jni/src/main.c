...
char mainProgramFilename[FILENAME_MAX] = "app.nx";
...
int main(int argc, char *argv[])
{
    ...
    if (runner_isOkay(&runner))
    {
        ...
        SDL_SetHint(SDL_HINT_GAMECONTROLLERTYPE,"");
        ...
        */
        ...
        SDL_GetCurrentDisplayMode(0, &display_mode);
			window = SDL_CreateWindow(NULL, 0, 0, display_mode.w, display_mode.h, SDL_WINDOW_FULLSCREEN|SDL_WINDOW_BORDERLESS|SDL_WINDOW_SHOWN|SDL_WINDOW_RESIZABLE|SDL_WINDOW_ALLOW_HIGHDPI);
...
{
    char *prefPath = SDL_GetPrefPath("moechofe", "Poink");
    if (prefPath)
    {
...
void updateScreenRect(int winW, int winH)
{
    double factor = fmax(1, fmin(winW / (double)SCREEN_WIDTH, winH / (double)SCREEN_HEIGHT));
    int nxScreenW = (double)SCREEN_WIDTH * factor;
    int nxScreenH = (double)SCREEN_HEIGHT * factor;
...
void setTouchPosition(int windowX, int windowY)
{
    coreInput.touchX = (windowX - screenRect.x) * SCREEN_WIDTH / (double)screenRect.w;
    coreInput.touchY = (windowY - screenRect.y) * SCREEN_HEIGHT / (double)screenRect.h;
}
